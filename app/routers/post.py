...
#                     limit:int = 10, skip:int = 0, search: Optional[str] = ""):
    ...
    #                models.Post.owner_id == current_user.id).limit(limit=limit)\
    #                .offset(skip).all()
    results = db.query(models.Post, func.count(models.Vote.post_id).label('votes'))\
                    .join(models.Vote, models.Vote.post_id == models.Post.id, 
                    isouter=True).group_by(models.Post.id)\
                    .filter(models.Post.title.contains(search)).limit(limit=limit)\
                    .offset(skip).all()
    return results

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=schemas.Post)
async def create_posts(post:schemas.PostCreate, response:Response, db:Session = Depends(get_db),
...
def get_post(id: int, response: Response, db: Session = Depends(get_db), 
    ...
    if post.Post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, 
                            detail='Not authorized to perform that operation')
    return post
